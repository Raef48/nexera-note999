...
export default function Signup() {
  ...
  const handleSignup = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // 2. Create profile in public.profiles table using RPC
      const { data: newProfile, error: profileError } = await insforge.database
        .rpc('upsert_profile', {
    ...
    <div className="min-h-screen bg-zinc-950 flex items-center justify-center p-4">
      <div className="max-w-md w-full space-y-6 sm:space-y-8 bg-zinc-900 p-6 sm:p-8 rounded-2xl border border-zinc-800 shadow-2xl">
        <div className="text-center space-y-2">
          ...
          <h1 className="text-2xl sm:text-3xl font-bold text-zinc-100">Join Nexera Note</h1>
          <p className="text-zinc-500 text-sm">Create your AI-powered workspace</p>
        </div>
